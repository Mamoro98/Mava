...
def get_learner_fn(
    ...
    def _update_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple]:
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple,task_id:int=0) -> Tuple:
            ...
            #     (*minibatches_list[0], prev_hs_minibatch_list[0]),
            # )
...
def learner_setup(
    ...
    def print_detailed_params(params_pytree):
        total_count = 0

        for path, leaf_array in jax.tree_util.tree_leaves_with_path(params_pytree):
            name = _path_to_str(path)
            count = leaf_array.size
            print(f"  {name}: {count} (shape: {leaf_array.shape})")
            total_count += count

        print(f"\nOverall Total Parameters: {total_count}")
        return total_count
    

    print_detailed_params(params)
    ...
    # broadcast = lambda x: jnp.broadcast_to(x, (config.system.update_batch_size, *x.shape))
    # learner_state = (params, opt_state, step_keys)
    # learner_state_bcast = tree.map(broadcast, learner_state)
    # replicate_learner = flax.jax_utils.replicate(learner_state_bcast, devices=jax.devices())

    # joint_replicated_hs = []
    # for init_hs in joint_hstates:
    #     hs_b = tree.map(broadcast, init_hs)
    #     hs_r = flax.jax_utils.replicate(hs_b, devices=jax.devices())
    #     joint_replicated_hs.append(hs_r)
